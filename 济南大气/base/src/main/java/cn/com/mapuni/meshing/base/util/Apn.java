...
import cn.com.mapuni.meshing.base.business.BaseApnInfo;
...
public class Apn {
	...
	public BaseApnInfo getCurrentApn() {
		...
		while (cr != null && cr.moveToNext()) {
			...
			LogUtil.d("carWifi", "---getCurrentApn-----=====" + apnInfo.getApnId());
		}		
		cr.close();
		return apnInfo;
	}
	...
	public Uri addApn(BaseApnInfo apnInfo) {
		...
		if (apnInfo != null) {
			...
			LogUtil.d("carWifi", "---addApn-uri----=====" + uri);
		}
		return uri;
	}
	...
	public int updateApn(BaseApnInfo apnInfo) {
		...
		LogUtil.d("carWifi", apnInfo.getApnId() + "---updateApn-----=====" + x);
		return x;
	}
