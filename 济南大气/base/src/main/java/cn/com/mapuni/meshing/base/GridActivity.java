...
import cn.com.mapuni.meshing.base.util.Apn;
...
public class GridActivity extends BaseActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (it.getBooleanExtra("isShortcut", false)) {
			...
			try {
				if (packageName.equals("")) {
					...
					businessObj = (IGrid) BaseObjectFactory
							.createHelperObject(ywl);
				} else if (packageName.equals("setting")) {
					businessObj = (IGrid) BaseObjectFactory
							.createSettingObject(ywl);
				} else if (packageName.equals("infoQuery")) {
					businessObj = (IGrid) BaseObjectFactory
							.createinfoQueryObject(ywl);
				}
		...
		super.setQueryButtonVisiable(true);
		...
		if (businessObj.getGridTitle().equals("综合查询"))
			...
			LogUtil.i("drawable-hdpi", "HashCode ：" + this.toString());
		GridView gridview = new LoadGridLayout(GridActivity.this, intent,
				businessObj, ydbg).loadGridLayout();
		/** 过去总布局的中间布局 */
	...
	protected void onRestoreInstanceState(Bundle savedInstanceState) {
		LogUtil.i("tag", "savedInstanceState");
		if (savedInstanceState == null) {
			LogUtil.i("tag", "savedInstanceState空");
		}
	 ...
	 */
	private class btnExitListener implements
