...
public class VideoOnlineActivity extends BaseActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
		...
		for (HashMap<String, Object> map : dataList) {
			...
			if ("1".equals(status)) {
				...
				LogUtil.i(TAG, (String)map.get("qymc") + (String)map.get("ip"));
			}
		}
		if (!initeSdk()) {
			this.finish();
    ...
    private boolean initeSdk() {
    	...
    	if (null == m_oHCNetSDK) {
    		LogUtil.e(TAG, "m_oHCNetSDK new is failed!");
    		return false;
    	}
    	
    	if (!m_oHCNetSDK.NET_DVR_Init()) {
    		LogUtil.e(TAG, "HCNetSDK init is failed!");
    		return false;
    	}
    	...
    	{
    		LogUtil.e(TAG,"PlayCtrl getInstance failed!");
    		return false;
    	}
    	return true;
	}
	...
	private void onSpinnerItemSelectedAction(int id) {
		...
		if (Net.checkNet(this)) {
			// 用户选择，获取对应IP
			LogUtil.i(TAG, "选择" + listText.get(id) + listIP.get(id));
	...
	private void onLoginAction() {
		...
		try {
			if(m_iLogID < 0) {
				...
				if (m_iLogID < 0) {
					//loginAgain();
					LogUtil.e(TAG, "This device logins failed!");
					return;
				}
				...
				if (oexceptionCbf == null) {
					//loginAgain();
					LogUtil.e(TAG, "ExceptionCallBack object is failed!");
				    return ;
				}
				
				if (!m_oHCNetSDK.NET_DVR_SetExceptionCallBack(oexceptionCbf)) {
					//loginAgain();
					LogUtil.e(TAG, "NET_DVR_SetExceptionCallBack is failed!");
			        return;
			    }
				
				LogUtil.i(TAG, "Login sucess ****************************1***************************");
			} else {
				// whether we have logout
				/*if (!m_oHCNetSDK.NET_DVR_Logout_V30(m_iLogID)) {//退出登录
					LogUtil.e(TAG, " NET_DVR_Logout is failed!");
					return;
				}
				m_iLogID = -1;*/
			}		
			...
			LogUtil.e(TAG, "error: " + err.toString());
		}
	}
	...
	private void onPreviewAction(){
		try {
			if(m_iLogID < 0) {
				LogUtil.e(TAG,"please login on device first");
				return ;
			}
			if(m_iPlayID < 0) {	
				if(m_iPlaybackID >= 0) {
					LogUtil.i(TAG, "Please stop palyback first");
					return;
				}
				RealPlayCallBack fRealDataCallBack = getRealPlayerCbf();
				if (fRealDataCallBack == null) {
				    LogUtil.e(TAG, "fRealDataCallBack object is failed!");
		            return;
				}
				
				int iFirstChannelNo = m_oNetDvrDeviceInfoV30.byStartChan;// get start channel no
				
				LogUtil.i(TAG, "iFirstChannelNo:" +iFirstChannelNo);
		       ...
		       // m_oHCNetSDK.NET_DVR_RealPlay_V30(arg0, arg1, arg2, arg3)
				if (m_iPlayID < 0) {
					//previewAgain();
					LogUtil.e(TAG, "NET_DVR_RealPlay is failed!Err:" + m_oHCNetSDK.NET_DVR_GetLastError());
				 	return;
				}
				int errorCode=m_oHCNetSDK.NET_DVR_GetLastError();
				LogUtil.i(TAG, "NetSdk Play sucess ***********************3***************************");									
			}
			else {
				//stopPlay();
			}				
	...
	private void stopPlay()
	{
		...
		{
			LogUtil.e(TAG, "StopRealPlay is failed!Err:" + m_oHCNetSDK.NET_DVR_GetLastError());
			return;
		}
        ...
        {
            LogUtil.e(TAG, "stop is failed!");
            return;
        }	
		
		if(!m_oPlayerSDK.closeStream(m_iPort))
		{
            LogUtil.e(TAG, "closeStream is failed!");
            return;
        }
		if(!m_oPlayerSDK.freePort(m_iPort))
		{
            LogUtil.e(TAG, "freePort is failed!");
            return;
        }
		m_iPort = -1;
		// set id invalid
	...
	private int loginDevice()
	{
		...
		{
			LogUtil.e(TAG, "HKNetDvrDeviceInfoV30 new is failed!");
			return -1;
		}
		...
		if (iLogID < 0)
		{
			LogUtil.e(TAG, "NET_DVR_Login is failed!Err:" + m_oHCNetSDK.NET_DVR_GetLastError());
			return -1;
		}
		
		LogUtil.i(TAG, "NET_DVR_Login is Successful!");
	...
	private void processRealData(int iPlayViewNo, int iDataType, byte[] pDataBuffer, int iDataSize, int iStreamMode) {
		int i = 0;
		LogUtil.i(TAG, "iPlayViewNo:" + iPlayViewNo + "iDataType:" + iDataType + "iDataSize:" + iDataSize);
		try {
			switch (iDataType) {
			...
			case HCNetSDK.NET_DVR_STD_VIDEODATA:
				if (iDataSize > 0 && m_iPort != -1) {
					...
					if (i == 400) {
						LogUtil.e(TAG, "inputData failed");
					}
				}
				break;
			default:
			...
			LogUtil.e(TAG, "processRealData Exception!err:" + e.toString());
		}
	}
