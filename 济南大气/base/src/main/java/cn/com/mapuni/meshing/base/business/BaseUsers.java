...
import cn.com.mapuni.meshing.base.util.PinyinOperator;
 ...
 */
public class BaseUsers extends BaseClass implements Serializable, IQuery,
		IList, IDetailed {

	/** 实体类的名字 */
	public static final String BusinessClassName = "BaseUsers";
	/** 获取该实体类列表样式的标题 */
	private  final String ListStyleName = "USERS";
	/** 获取该实体类数据详情样式的标题 */
	private  final String DetailedStyleName = "PC_USERS";
	/** 获取该实体类查询样式的标题 */
	private  final String QueryStyleName = "USERS";
	/** 该实体类所在表的主键 */
	private  final String primaryKey = "userid";
	/** 查询该实体类的相关信息所用的表名 */
	private  final String tableName = "V_Users";
	/** 该实体类在显示详情是所用的名字标题 */
	private final String DetailedTitleText = "用户信息";
	/** 该实体类在显示列表的时候所用的名字标题 */
	private String GridTitleText = "通讯录";
	...
	public BaseUsers(Context context) {
		try {
	...
	public HashMap<String, Object> getDetailed(String id) {
		...
		Object mySql = styleDetailed.get(styleDetailed.size() - 1).get(
				"queryHint")
				+ SqliteUtil.getInstance().getFilterForSqlDetail(
						primaryKeyMap);
