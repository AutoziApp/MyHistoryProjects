 ...
 */
package cn.com.mapuni.meshing.base.viewflow;
...
public class ViewFlow extends AdapterView<Adapter> {
	...
	/**getview方法调用偏移  例如，默认从item0开始加载，第一次初始调用getview方法几次？1--》向左和向右偏移1*/
	private int mSideBuffer = 2;
	private Scroller mScroller;
	/**帮助你追踪一个touch事件（flinging事件和其他手势事件）的速率。当你要跟踪一个touch事件的时候，使用obtain()方法得到这个类的实例，
	 * 然后 用addMovement(MotionEvent)函数将你接受到的motion event加入到VelocityTracker类实例中。当你使用到速率时，
	 * 使用computeCurrentVelocity(int)初始化速率的单位，并获得当前的事件的速率，
	 * 然后使用getXVelocity() 或getXVelocity()获得横向和竖向的速率*/
	private VelocityTracker mVelocityTracker;
	private int mTouchState = TOUCH_STATE_REST;
	/**触发一次event事件后最后记录手指停留在x坐标的位置*/
	private float mLastMotionX;
	/**认为是滑动事件的最小距离*/
	private int mTouchSlop;
	/**fling的最大速度，单位是每秒多少像素 */
	...
	/**viewflow状态监听，fling或者rest*/
	private ViewStateListener  mViewStateListener;
	private Adapter mAdapter;
	private int mLastScrollDirection;
	private AdapterDataSetObserver mDataSetObserver;
	private FlowIndicator mIndicator1, mIndicator2;
	...
	public ViewFlow(Context context, AttributeSet attrs) {
		...
		mSideBuffer = styledAttrs.getInt(R.styleable.ViewFlow_sidebuffer, 2);//默认为2，加载时黑屏、改为1，但是存在闪出小人的问题
		init();
	}
	...
	private void init() {
		...
		final ViewConfiguration configuration = ViewConfiguration
				.get(getContext());
		mTouchSlop = configuration.getScaledTouchSlop();//认为是滑动事件的最小距离
		mMaximumVelocity = configuration.getScaledMaximumFlingVelocity();//用来初始化fling的最大速度，单位是每秒多少像素 
	}
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_UP:
			if (mTouchState == TOUCH_STATE_SCROLLING) {
				...
				/*参数：maxVelocity  该方法所能得到的最大速度，这个速度必须和你指定的units使用同样的单位，而且 必须是整数。
				（也就是，你指定一个速度的最大值，如果计算超过这个最大值，就使用这个最大值，否则，使用计算的的结果*/
				velocityTracker.computeCurrentVelocity(1000, mMaximumVelocity);
				
				int velocityX = (int) velocityTracker.getXVelocity();//得到x轴滑动速度
				...
				if (mVelocityTracker != null) {
					mVelocityTracker.recycle();//回收addMovement
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			final int xDiff = (int) Math.abs(x - mLastMotionX);//求绝对值
			...
			if (mTouchState == TOUCH_STATE_SCROLLING) {
				...
				if(mViewStateListener!=null){
					mViewStateListener.onStateChange(mTouchState);
				}
				return true;
			}
			break;
	...
	protected void onScrollChanged(int h, int v, int oldh, int oldv) {
		...
		if (mIndicator1 != null) {
			...
			int hPerceived = h + (mCurrentAdapterIndex - mCurrentBufferIndex)
					* getWidth();
			mIndicator1.onScrolled(hPerceived, v, oldh, oldv);
		}
		if (mIndicator2 != null) {
			/*
			 * The actual horizontal scroll origin does typically not match the
			...
			int hPerceived = h + (mCurrentAdapterIndex - mCurrentBufferIndex)
					* getWidth();
			mIndicator2.onScrolled(hPerceived, v, oldh, oldv);
		}
	}
	...
	/**
	 * Description:ViewFlow手势状态监听，down、move、up、rest
	 * @param l
	 * @author wanglg
	 * @Create at: 2013-6-9 下午2:58:02
	 */
	public void setOnViewStateListener(ViewStateListener l) {
		mViewStateListener = l;
	}
	...
	public void setFlowIndicator(FlowIndicator... flowIndicator) {
		if(flowIndicator.length > 0){
			mIndicator1 = flowIndicator[0];
			mIndicator1.setViewFlow(this);
			if(flowIndicator.length > 1){
				mIndicator2 = flowIndicator[1];
				mIndicator2.setViewFlow(this);
			}
		}
	}
	...
	public void setSelection(int position) {
		...
		if (mIndicator1 != null) {
			mIndicator1.onSwitched(mLoadedViews.get(mCurrentBufferIndex),
					mCurrentAdapterIndex);
		}
		if (mIndicator2 != null) {
			mIndicator2.onSwitched(mLoadedViews.get(mCurrentBufferIndex),
					mCurrentAdapterIndex);
		}
		if (mViewSwitchListener != null) {//执行ViewSwitchListener监听，回调onSwitched方法
			mViewSwitchListener
	...
	/**
	 * Description: 调用adapter的getview方法，返回子view对象
	 * @param position
	 * @param addToEnd
	 * @param convertView
	 * @return
	 * @author Administrator
	 * @Create at: 2013-6-13 上午11:41:00
	 */
	...
	private void logBuffer() {
		...
		LogUtil.d("viewflow", "Size of mLoadedViews: " + mLoadedViews.size() +
				"X: " + mScroller.getCurrX() + ", Y: " + mScroller.getCurrY());
		LogUtil.d("viewflow", "IndexInAdapter: " + mCurrentAdapterIndex
