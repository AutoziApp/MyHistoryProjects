...
public class UpdtaeScriptDownloadConfigService extends Service {
		...
		new Thread() {
			...
			public void run() {
				...
				if (Net.checkURL(urlString + PathManager.APK_CODE_URL)) {
					...
					if (autopudate.JudgeNewVerson(urlString + PathManager.APK_CODE_URL,UpdtaeScriptDownloadConfigService.this) == 1) {/** 有新版本则弹出更新对话框  */
						hasNewAPK=true;
					}
	...
	public void onDestroy() {
		...
		super.onDestroy();
		LogUtil.i(TAG, "UpdtaeScriptDownloadConfigService服务销毁--->");
	}
}
