...
import com.vhtableview.VHBaseAdapter;

import java.util.ArrayList;

import cn.com.mapuni.chart.meshingtotal.DeptCheckDetailActivity;
...
public class VHTableAdapter implements VHBaseAdapter {
    ...
    private DepCheckInfoBean depCheckInfoBean;

    public VHTableAdapter(Context context, ArrayList<String> titleData, ArrayList<ArrayList<String>> dataList,DepCheckInfoBean depCheckInfoBean) {
        this.context=context;
        this.titleData=titleData;
        this.dataList=dataList;
        this.depCheckInfoBean=depCheckInfoBean;
    }
    ...
    public void OnClickContentRowItem(int row, View convertView) {
        Intent intent=new Intent(context,DeptCheckDetailActivity.class);

        intent.putExtra("StartTime",depCheckInfoBean.getCheckTimespan().substring(0,19));
        intent.putExtra("EndTime",depCheckInfoBean.getCheckTimespan().substring(20));
        intent.putExtra("DeptID",depCheckInfoBean.getData().get(row).getDeptID());
