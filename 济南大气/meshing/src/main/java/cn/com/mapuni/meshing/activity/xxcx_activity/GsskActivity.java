...
import cn.com.mapuni.meshing.model.DbData;
...
public class GsskActivity extends BaseMapActivityTDT implements ViewCallback,
		View.OnClickListener, OnMarkerClickListener, OnItemClickListener {
	...
	private void addMyLocationAndMarders() {
		...
		for (int i = 0; i < datas.size(); i++) {
			...
			if (aqi <= 51) {
				...
				gsskTextLeft.setText(aqi + "");
				gsskTextLeft.setBackgroundResource(R.drawable.cha_left);
				gsskTextRight.setText(dbData.getCityName());
				gsskTextRight.setBackgroundResource(R.drawable.cha_right);
	...
	private void initPopView() {
		mPopView = LayoutInflater.from(mContext).inflate(R.layout.popview_gssk,
				null);
	...
	private void locationAndview(MarkerOverlay marker) {
		...
		mMapView.updateViewLayout(mPopView,
				new MapView.LayoutParams(LayoutParams.WRAP_CONTENT,
						...
						-50, MapView.LayoutParams.BOTTOM_CENTER));
		for (int i = 0; i < datas.size(); i++) {
			DbData dbData = datas.get(i);
