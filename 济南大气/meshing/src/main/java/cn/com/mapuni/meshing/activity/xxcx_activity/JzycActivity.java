...
public class JzycActivity extends BaseMapActivityTDT implements ViewCallback,
	...
	void initView() {
		...
		marker = mContext.getResources().getDrawable(R.drawable.yangchen);
		mPopView = LayoutInflater.from(mContext).inflate(R.layout.popview_jzyc,
				null);
		biaoti1.setText("企业单位");
		biaoti2.setText("地址");
		biaoti3.setText("污染状态");
		biaoti4.setText("");
		biaoti4.setVisibility(View.GONE);
		biaoti5.setText("操作");

		List<DbNews> data = new ArrayList<DbNews>();
		data.add(new DbNews("济南二机床集团有限公司", "槐荫区", "正常", "", "详情"));
		data.add(new DbNews("山东华森混凝土有限公司", "历下区", "正常", "", "详情"));
		data.add(new DbNews("济南华光电镀有限公司", "市中区", "不正常", "", "详情"));
