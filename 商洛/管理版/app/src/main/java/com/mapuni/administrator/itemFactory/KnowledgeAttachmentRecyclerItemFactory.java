...
import com.mapuni.administrator.utils.ToastUtil;
...
public class KnowledgeAttachmentRecyclerItemFactory extends AssemblyRecyclerItemFactory<KnowledgeAttachmentRecyclerItemFactory.AttachmentRecyclerItem> {
    private EventListener mEventListener;

    private boolean role;

    public KnowledgeAttachmentRecyclerItemFactory(Context context, boolean role) {
    ...
    private static class EventProcess implements EventListener {
        ...
        private void deleteAccount(String uuid) {
            ...
            NetManager.deleteKnowledgeAttachment(sessionId, uuid, new StringCallback() {
                @Override
...
//                    clearLoginMsg();
                    ...
                    EventBus.getDefault().post(new MessageEvent("KnowledgedeleteSuccess"));
                }
            });
        }
        ...
        public void getAttachUrl(final AttachmentBean.RowsBean bean) {
            ...
            String sessionId = (String) SPUtils.getSp(context, "sessionId", "");
            NetManager.getKnowledgeAttachUrl(sessionId, taskAttachmentUuid, new StringCallback() {
                @Override
                public void onError(Call call, Exception e, int id) {
                    ...
                    try {
                        ...
                        if (jsonObject.optString("status").equals("200")) {
                            ...
                            if (TypeUtil.isImageType(extension)) {//如果是图片格式
                                ...
                                try {
                                    String urlUTF = URLDecoder.decode(url, "UTF-8").toString();
                                    FileDownUtil.getInstance(context).downLoadWord(urlUTF, bean.getName());
                                } catch (UnsupportedEncodingException e) {
                                    e.printStackTrace();
                                }
                            } else if (TypeUtil.isMP3Type(extension)) {//mp3/4格式 下载
