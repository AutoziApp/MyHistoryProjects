...
public class SqliteUtil {
	...
	public SQLiteDatabase db = SQLiteDatabase.openOrCreateDatabase(
			DB_FULL_PATH, null);
	...
	public SQLiteDatabase getConnection() {
		...
		try {
			...
			fe.printStackTrace();
			if (context != null) {
				Toast.makeText(context, "缺少数据库文件程序无法运行", Toast.LENGTH_LONG)
						.show();
			}
	...
	public String[] getDBTables() {
		...
		try {
			...
			SQLiteException se = new SQLiteException(
					"数据库查询异常：获取当前数据库所有表名失败(getDBTables)\n" + e.getMessage());
	...
	public String[] getTableColumns(String table) throws Exception {
		...
		try {
			...
			SQLiteException se = new SQLiteException(table
					+ "获取所有列名查询异常：(getTableColumns)\n" + e.getMessage());
			RecordLog.WriteCaughtEXP(se, TAG);
			se.printStackTrace();
			throw new SQLiteException("数据库查询异常");
		}
		...
		LogUtil.v(TAG, "getTableColumns " + table + " columns --> "
	...
	public String getTablePrimaryKey(String table) throws Exception {
		...
		LogUtil.v(TAG, "getTablePrimaryKey " + table + " primary key --> "
				+ primary);
		return primary;
	}
	...
	public ArrayList<HashMap<String, Object>> queryAllTableDataByTableReturnArrayListHashMap(
		...
		LogUtil.v(TAG,
				"queryAllTableDataByTableReturnArrayListHashMap parameters : table --> "
						+ table);
		...
		try {
			...
			SQLiteException se = new SQLiteException(
					table
							+ "数据库查询异常：(queryAllTableDataByTableReturnArrayListHashMap)\n"
							+ e.getMessage());
			RecordLog.WriteCaughtEXP(se, TAG);
			throw se;
		}
	...
	public Cursor queryBySql(String sql) {
		...
		try {
			...
			SQLiteException se = new SQLiteException("sql语句错误：" + sql
					+ "数据库查询异常：(queryBySql)\n" + e.getMessage());
			RecordLog.WriteCaughtEXP(se, TAG);
	 ...
	 */
	public Cursor queryTableDataByTableId(String table,
			HashMap<String, String> primaryKey) throws SQLiteException {
		String key = primaryKey.get("key");
		String keyValue = primaryKey.get("keyValue");

		LogUtil.v(TAG, "queryTableDataByTableId parameters : table --> "
				+ table + "; primaryKey --> " + primaryKey.toString());
		...
		try {
			...
			SQLiteException se = new SQLiteException(table + "sql语句为：" + sql
					+ "," + "数据库查询异常：\n" + e.getMessage());
			RecordLog.WriteCaughtEXP(se, TAG);
			throw se;
		}
	 ...
	 */
	public HashMap<String, Object> queryTableDataByTableIdReturnHashMap(
			String table, HashMap<String, String> primaryKey) {

		LogUtil.v(TAG,
				"queryTableDataByTableIdReturnHashMap parameters : table --> "
						+ table + "; primaryKey --> " + primaryKey.toString());
	 ...
	 * @throws FileNotFoundException
	 */
	public long insert(ArrayList<ContentValues> values, String table)
			throws SQLiteException {
		db = getConnection();
	 ...
	 * @throws FileNotFoundException
	 */
	public long insertNoTransaction(List<ContentValues> values, String table)
			throws SQLiteException {
		long count = 0;
		db = getConnection();
		db.beginTransaction();
		try {
			for (ContentValues contentValues : values) {
				if ("PC_Users".equals(table)) {
					if (db.replace(table, null, contentValues)!=-1) {
						count++;
					}
				}else{
					if (db.insert(table, null, contentValues)> 0) {
			...
			OtherTools
					.showLog("----------insertNoTransaction----------数据库插入异常--------------------");
		} finally {
			db.endTransaction();
			db.close();
	...
	public void delete(String key, ArrayList<String> keyValue, String table) {
		...
		try {
			...
			String deleteSql = "DELETE FROM " + table + " WHERE 1=1 AND " + key
					+ " in (" + whereString + ")";
			...
			db.endTransaction();
			RecordLog.WriteCaughtEXP(new SQLiteException("表" + table
					+ "删除数据异常：\n" + e.getMessage()), TAG);
	 ...
	 *            要操作的表名
	 */
	public void deleteNoTransaction(String primaryKey,
			ArrayList<String> keyValue, String table) {
		...
		try {
			...
			db = getConnection();
			String whereString = GetClauseSQL(valueStrings);
			String deleteSql = "DELETE FROM " + table + " WHERE 1=1 AND "
					+ primaryKey + " in (" + whereString + ")";
			db.execSQL(deleteSql);
	 ...
	 * @throws FileNotFoundException
	 */
	public void deleteAllTableDataByTables(String tables)
			throws SQLiteException {
		...
		try {
			...
			RecordLog.WriteCaughtEXP(
					new SQLiteException("数据库删除数据异常：\n" + e.getMessage()), TAG);
	...
	public void execute(String sql) throws SQLiteException {
		...
		try {
			...
			db.endTransaction();
			RecordLog.WriteCaughtEXP(new SQLiteException("sql:" + sql
					+ ",SQL语句异常：(execute)\n" + e.getMessage()), TAG);
	 ...
	 * @return
	 */
	public static ArrayList<ContentValues> convertArrayListHashMapToArrayListContentValues(
		...
		try {
			...
			for (HashMap<String, Object> dataRow : data) {
				Iterator<Map.Entry<String, Object>> dataRowIterator = dataRow
						.entrySet().iterator();
				contentValues = new ContentValues();
				while (dataRowIterator.hasNext()) {
					Map.Entry<String, Object> entry = (Map.Entry<String, Object>) dataRowIterator
							.next();
	...
	public ArrayList<HashMap<String, Object>> convertCursorToArrayListHashMap(
		...
		try {
			...
			RecordLog.WriteCaughtEXP(
					new SQLiteException("用户解析游标数据出错：\n" + e.getMessage()), TAG);
	...
	public HashMap<String, Object> convertCursorToHashMap(Cursor data_cursor) {
		...
		try {
			...
			while (data_cursor.moveToNext()) {
				for (String col : tableColumon) {
					/* 列值如果为null，则赋“” */
					String colValue = data_cursor.getString(data_cursor
							.getColumnIndex(col));
					if (colValue == null) {
						colValue = "";
					}
	 ...
	 */
	public ArrayList<HashMap<String, Object>> getUnionQuerys(String sqlstr,
			HashMap<String, Object> conditions, String limit, String order,
			String str) {
		...
		if (conditions != null && conditions.size() > 0) {
			...
			while (iterator.hasNext()) {
				...
				if (!condition.getValue().equals("")) {
					sql.append("and (" + condition.getKey().trim() + " like '%"
							+ condition.getValue() + "%')");
				}
	...
	public HashMap<String, Object> getDetailed(String table,
		...
		try {
			HashMap<String, Object> data = queryTableDataByTableIdReturnHashMap(
					table, primaryKey);
	 ...
	 * @return
	 */
	public boolean synchronizeOneTableUpdateOrFetchAll(
	 ...
	 * @Create at: 2013-9-5 下午5:41:14
	 */
	public ArrayList<HashMap<String, Object>> getUnionQuery(String sqlstr,
			HashMap<String, Object> conditions, String limit, String order) {
		StringBuilder sql = new StringBuilder(sqlstr);
		if (conditions != null && conditions.size() > 0) {
			Iterator<Entry<String, Object>> iterator = conditions.entrySet()
	 ...
	 * @return
	 */
	public int getNeedNumByCondition(String table,
			HashMap<String, Object> conditions) {
