...
public class JsonHelper {
	public static <T> T paseJSON(String jsonString, Class<T> cls) {
		T t = null;
		try {
			Gson gson = new Gson();
			t = gson.fromJson(jsonString, cls);
		} catch (Exception e) {
			System.out.println("------------------------Gson to object error---------");
		}
		return t;
	}
	...
	public static <T> ArrayList<T> paseJSONReturnList(String jsonString, Class<T> cls) {
		T t = null;
						...
						try {
							...
							if (jt != JsonToken.NULL) {
								...
								if (jt == JsonToken.STRING) {
									...
									hashmap.put(tagName, reader.nextBoolean() + "");
								}
							} else {
								reader.skipValue();
								hashmap.put(tagName, "");
	...
	public static String listToJSONXin(ArrayList<HashMap<String, Object>> data) {
		StringBuffer sb = new StringBuffer();
//		sb.append("[");
		for (HashMap<String, Object> m : data) {
			sb.append("{");
