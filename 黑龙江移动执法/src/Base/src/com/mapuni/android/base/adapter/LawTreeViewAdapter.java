...
public class LawTreeViewAdapter extends BaseExpandableListAdapter {
	...
	public View getChildView(int groupPosition, int childPosition,
		...
		//wsc 增加显示文件夹下文件的数量
		File tempf=new File(filepath + "/复函");
		File [] tempfile =tempf.listFiles();
		if(tempfile!=null){
		String sfh="复函" + "(" + Integer.toString(tempfile.length) +")" ;
		fuhan.setText(sfh);
		}
		
		File tempxx=new File(filepath + "/现行法律");
		File [] temfilexx =tempxx.listFiles();
		if(temfilexx!=null){
		String sfhxx="现行法律" + "(" + Integer.toString(temfilexx.length) +")" ;
		xxfg.setText(sfhxx);
		}
