...
public class SlideView extends LinearLayout implements OnPageChangeListener, OnClickListener {
	...
	/** 选中背景色 */
	private int selectBackground = R.color.rwgl_fl;
	/** 未选中背景色 */
	private int loseBackground = R.color.rwgl_fl;
	...
	public SlideView(Context context, int margin) {
		...
		barItemH = (int) (dm.density * 45);
		barItemW = (int) (dm.density * 100);
		...
		MarginLayoutParams layoutParams = new MarginLayoutParams(LayoutParams.WRAP_CONTENT, dip2px(mContext, 38));

		margin = dip2px(mContext, margin);// 转换像素为DP
		layoutParams.setMargins(margin, margin, margin, margin);
		addView(CreateBar(), new LinearLayout.LayoutParams(layoutParams));
		addView(CreatePager());
	}
	...
	private View CreateBar() {
		if (indicateBar == null && scrollbar == null) {
			...
			scrollbar.setLayoutParams(new LayoutParams(LayoutParams.MATCH_PARENT, dip2px(mContext, 38)));
	...
	public void SetBarFocused(int index) {
		...
		for (int i = 0; i < count; i++) {
			...
			if (i == index) {
				...
				((TextView) (layout.findViewById(textViewID))).setTextColor(Color.parseColor("#ffffff")/*R.color.white*/);
			} else {
				...
				((TextView) (layout.findViewById(textViewID))).setTextColor(Color.parseColor("#b2bded"));
			}
	...
	public void AddPageView(SlideOnLoadAdapter adapter, String indiText) {
		...
		// 隐藏菜单栏滚动条
		if (null == indiText || "".equals(indiText)) {
			indicateBar.setLayoutParams(new LayoutParams(0, 0));
