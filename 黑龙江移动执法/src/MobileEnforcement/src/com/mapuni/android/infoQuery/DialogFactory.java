 ...
 *         Description
 */
public class DialogFactory extends BaseClass {
	 ...
	 *            要查询的业务类里的查询标题
	 */
	public DialogFactory(Context context, String title, String queryname,
			IQuery queryObj) {
		this.title = title;
		...
		for (int x = 0; x < itemCount; x++) {
			...
			if (extraInfoForQuery.get(x).get("style").equals("spinner")) {
					...
					if (regionsData != null && regionsData.size() > 0) {
						...
						try {
							if (strs.length != 0) {
								...
								strSql += "'" + strs[strs.length - 1] + "', '231000000')";
				...
				/** Spinner获取所在区县的内容 */
					...
					/** 获取下拉列表的内容 */
					adapterlist = getAdapterList(extraInfoMap.get("datasource")
							.toString(), querycondition);
					/** 判断配置文件是否配置正确 */
					if (adapterlist == null) {
			...
			if (isSpinner) {
				...
				if (edittext.getText().toString().trim().length() <= 0)
					...
					if ("qymc".equals(conditionName)) {
						conditionName = "T_WRY_QYJBXX.qymc";
					}
