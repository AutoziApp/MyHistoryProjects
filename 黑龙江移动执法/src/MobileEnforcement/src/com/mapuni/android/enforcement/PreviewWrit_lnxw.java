...
public class PreviewWrit_lnxw {
	...
	public PreviewWrit_lnxw(String rWBH, String qyid,String guid, String tYPE, Activity a) {
		super();
		RWBH = rWBH;
		GUID = qyid;
		TYPE = tYPE;
	...
	public String create() {
		Log.i("info", "notsimple");
		if (RWBH == null || GUID == null) {
			handler.sendEmptyMessage(NO_RWBH_QYDM);
		}
		path = path + RWBH + "/" + GUID;

		String sqlxw = "select * from T_ZFWS_xwbl where taskid = '" + RWBH + "' " + "and surveyentcode = '" + GUID + "' and type = '" + TYPE + "'and guid = '"+guid+"'";

		String sqlwd = "select distinct  taskid,entid,wtnr,result  from T_ZFWS_XWJLWD where taskid = '" + RWBH + "' and entid = '" + GUID + "' and billid = '"+guid+"'";
		...
		try {
			...
			InputStream is = manager.open("hljxw3.html");
...
//		}
		...
		if ("黑龙江省".equals(topString)) {
			text = text.replace("{DutyPeopleEntpriseName1}",topString+"环境保护厅" );
		}else{
			text = text.replace("{DutyPeopleEntpriseName1}", topString+"环境保护局");
		}
	...
	private String question1(String address) {
		String a = " <tr ><td width=\"800px\" style=\"text-indent:2em;\">" + "<font style=\"font-weight:bold;font-size:20px\"></font>"
				+ "<font style=\"text-decoration:underline;word-break: break-all; word-wrap:break-word;line-height:35px;font-size:20px;\">"
	...
	private String answer1(String s) {
		...
		String b = " <tr ><td width=\"800px\">"
				+ "<font style=\"font-weight:bold;font-size:20px\"></font><font style=\"text-decoration:underline;word-break: break-all; word-wrap:break-word;line-height:35px;font-size:20px;\">"
				+ "答：清楚了,不需要回避。" + s + "</font>" + "  </td></tr>";
		return "";
	}
