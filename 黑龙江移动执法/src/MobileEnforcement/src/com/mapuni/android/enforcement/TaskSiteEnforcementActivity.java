	...
	private final Handler handler = new Handler() {
		...
		public void handleMessage(android.os.Message msg) {
			...
			case DATABASE_INSERT_EXCEPTION:
				...
				Toast.makeText(TaskSiteEnforcementActivity.this, "数据保存异常", Toast.LENGTH_SHORT).show();
				break;
			...
			case DATA_QUERY_SUCCESS:
				...
				if (task_name_tv.getText().toString().trim().equals("现场监察记录单")) {

					qdjcList = new ArrayList<HashMap<String, Object>>();
					HashMap<String, Object> hashmap = new HashMap<String, Object>();
					hashmap.put("code", "1");
					hashmap.put("name", "基本情况");
					qdjcList.add(hashmap);
					hashmap = new HashMap<String, Object>();
					hashmap.put("code", "2");
					hashmap.put("name", "污染治理设施建设运行情况");
					qdjcList.add(hashmap);
					hashmap = new HashMap<String, Object>();
					hashmap.put("code", "3");
					hashmap.put("name", "污染物排放情况");
					qdjcList.add(hashmap);
					hashmap = new HashMap<String, Object>();
					hashmap.put("code", "4");
					hashmap.put("name", "存在环境问题及意见");
					qdjcList.add(hashmap);
...
//							if (qyzts.equals("1")) {
						...
						askTv.setOnLongClickListener(new OnLongClickListener() {
							public boolean onLongClick(View v) {
								...
								new AlertDialog.Builder(TaskSiteEnforcementActivity.this).setIcon(R.drawable.yutu).setTitle("提示信息").setMessage("是否删除该条问答记录")
										.setPositiveButton("是", new OnClickListener() {
											...
											public void onClick(DialogInterface dialog, int which) {
												if (qyzts.equals("3")) {
													//BYK RWZT
//													if (qyzts.equals("1")) {
													Toast.makeText(TaskSiteEnforcementActivity.this, "已取证企业不能删除记录", Toast.LENGTH_SHORT).show();
													return;
												}

												task_site_enfor_wd_layout.removeView(wdLayout);
												Toast.makeText(TaskSiteEnforcementActivity.this, "删除成功", Toast.LENGTH_SHORT).show();
...
//													if (qyzts.equals("1")) {
			...
			case THE_USER_OR_ENT_NOT_EXIST:
				...
				Toast.makeText(TaskSiteEnforcementActivity.this, "该用户或者该企业不存在 ", Toast.LENGTH_SHORT).show();
				break;
			...
			case DATA_UPDATE_EXCEPTION:
				...
				Toast.makeText(TaskSiteEnforcementActivity.this, "数据更新失败 ", Toast.LENGTH_SHORT).show();
				break;
			...
			case DATA_UPDATE_SUCCESS:
				...
				Toast.makeText(TaskSiteEnforcementActivity.this, "数据更新成功 ", Toast.LENGTH_SHORT).show();
			...
			case PREVIEW_SAVE_DATA_SUCCESS:
				...
				if (task_name_tv.getText().toString().contains("勘察")) {
					...
					if (yon.equals("y")) {
						...
						Toast.makeText(TaskSiteEnforcementActivity.this, "暂无数据", Toast.LENGTH_SHORT).show();
					}
...
//					PanduanDayin.startprintshare(TaskSiteEnforcementActivity.this,file.getAbsolutePath());
				...
				} else if (task_name_tv.getText().toString().contains("询问")) {
				...
				} else if (task_name_tv.getText().toString().equals("环境监察通知书")) {
					...
					if (yon.equals("y")) {
						File file1 = new File(plnjstzs.getPath() + "/第1页.html");
						if (!PanduanDayin.appIsInstalled(TaskSiteEnforcementActivity.this, "com.dynamixsoftware.printershare")) {
							PanduanDayin.insatll(TaskSiteEnforcementActivity.this);
							return;
						}
	...
	OnTimeSetListener agin_time = new OnTimeSetListener() {
		...
		public void onTimeSet(TimePicker view, int hourOfDay, int minute) {
			...
			接受调查时间.setText(java.text.DateFormat.getDateTimeInstance(java.text.DateFormat.DEFAULT, java.text.DateFormat.MEDIUM, Locale.CHINESE).format(dateAndTime.getTime()));
			接受调查时间.setTag(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(dateAndTime.getTime()));// 2014-05-30
			// 02:00:35
		}
	};
...
//				task_surveyaddress_lay.setVisibility(View.VISIBLE);
				...
				task_surveypeoplename_one_tv.setText("检查人姓名:");
//				task_dutypeople_tv.setText("现场负责人姓名:");
	...
	public void together_people_click(View view) {
		...
		try {
			...
			if(TextUtils.isEmpty(depparentid)) {
				Toast.makeText(TaskSiteEnforcementActivity.this,"查询人员时出错!请同步人员表!",Toast.LENGTH_SHORT).show();
				return ;
			}
		} catch (Exception e) {
		}
		...
		for(HashMap<String, Object> map:depData){
			if(map.get("depname").equals("审核部门"))
		...
		if(!hasChildData){
			Toast.makeText(TaskSiteEnforcementActivity.this,"人员信息不完整!请同步人员表!",Toast.LENGTH_SHORT).show();
		}
		...
		builder.setPositiveButton("确定", new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				if (linkedList.size() < 2) {
					...
					Toast.makeText(TaskSiteEnforcementActivity.this, "除了自己，请至少再选择一位执法人！", Toast.LENGTH_LONG).show();
					return;
				}
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				if (task_name_tv.getText().toString().trim().equals("调查询问笔录")) {
					...
					return;
				} else if (task_name_tv.getText().toString().trim().equals("现场监察记录单")) {
...
//			surveyStr = xwrxmStr.split(",");
	...
	/** 获取输入的内容 */
	public void getData() {
		task_companyaddress_edt_str = task_company_address_edt.getText().toString().trim();// 调查地址
		// task_checkpeople_position_edt.setText("");//
		// 检查人职务task_surveyaddress_edt

		task_dutypeople_age_edt_str = task_dutypeople_age_edt.getText().toString().trim();// 被询问人的年龄

		task_natruepeople_edt_str = task_natruepeople_edt.getText().toString().trim();// 被询问自然人的姓名
		task_natruepeople_age_edt_str = task_natruepeople_age_edt.getText().toString().trim();// 被询问自然人年龄
		task_natruepeopleoffice_edt_str = task_natruepeopleoffice_edt.getText().toString().trim();// 被询问自然人职务
		task_natruepeopletel_edt_str = task_natruepeopletel_edt.getText().toString().trim();// 被询问自然人电话
		task_natruepeopleaddress_edt_str = task_natruepeopleaddress_edt.getText().toString().trim();// 被询问自然人地址

		task_surveyaddress_edt_str = task_surveyaddress_edt.getText().toString().trim();
		task_checkpeople_phone_edt_str = task_checkpeople_phone_edt.getText().toString().trim();// 法人代表电话
		task_checkpeople_zip_edt_str = task_checkpeople_zip_edt.getText().toString().trim();// 邮政编码
		task_business_license_edt_str = task_business_license_edt.getText().toString().trim();// 营业执照注册号
		task_organization_edt_str = task_organization_edt.getText().toString().trim();// 组织机构代码

		task_entcode_edt_str = task_entcode_edt.getText().toString().trim();// 污染源(企业)名称
		task_enforcement_certificate_no_edt_str = task_enforcement_certificate_no_edt.getText().toString().trim();// 检查人执法证号
		task_work_units_edt_str = task_work_units_edt.getText().toString().trim();// 检查人的工作单位
		//修改单位 byk   Global.getGlobalInstance().getUserUnit()  为NULL
	//	task_work_units_edt_str = Global.getGlobalInstance().getUserUnit();
		task_recordpeopleid_enforcement_certificate_no_edt_str = task_recordpeopleid_enforcement_certificate_no_edt.getText().toString().trim();// 记录人执法证号
...
//		task_recordpeopleid_work_units_edt_str = task_recordpeopleid_work_units_edt.getText().toString();// 记录人的工作单位
	...
	//	task_recordpeopleid_work_units_edt_str = Global.getGlobalInstance().getUserUnit();
		...
		task_checkpeople_edt_str = task_checkpeople_edt.getText().toString().trim();// 法人代表
		task_dutypeople_edt_str = task_dutypeople_edt.getText().toString().trim();// 现场负责人
		task_dutypeopleoffice_edt_str = task_dutypeopleoffice_edt.getText().toString().trim();// 现场负责人职务
		task_dutypeoplerelation_edt_str = task_dutypeoplerelation_edt.getText().toString().trim();// 现场负责人与本案关系
		task_dutypeopletel_edt_str = task_dutypeopletel_edt.getText().toString().trim();// 现场负责人的电话
		task_otherpeopleaddress_edt_str = task_otherpeopleaddress_edt.getText().toString().trim();// 其他参加人员的姓名及工作单位
		task_confirm_body_edt_str = task_confirm_body_edt.getText().toString().trim();// 确认正文
		task_sitecondition_edt_str = task_sitecondition_edt.getText().toString().trim();// 现场情况

		task_surveypeoplename_btn_str = task_surveypeoplename_btn.getText().toString().trim();// 获得检查人名称
		if (recordList != null && recordList.size() > 0) {
			task_recordpeopleid_sp_str = task_recordpeopleid_sp.getSelectedItem().toString().trim(); // 获得记录人的姓名
		} else {
			task_recordpeopleid_sp_str = "";
		}
		if (dengerList != null && dengerList.length > 0) {
			task_dutypeople_gender_spinner_str = button_xunwen.getText().toString().trim();// 获取被询问人的性别
		} else {
			task_dutypeople_gender_spinner_str = "";
		}

		if (record != null && record.length > 0) {
			task_naturepeople_gender_spinner_str = button_ziranren.getText().toString().trim();// 获取被询问自然人性别
		} else {
	...
	private void saveUpdateData(final int stateFlag) {
		if (!bNeedSave()) {
			if (stateFlag == SCENE_UPDATE_DATA_SUCCESS) {
				Toast.makeText(TaskSiteEnforcementActivity.this, "任务已上传成功", 0).show();
				return;
			}
			Message msg = handler.obtainMessage();
			msg.arg1 = stateFlag;
				...
				try {
					...
					if (task_name_tv.getText().toString().trim().equals("调查询问笔录")) {
						...
						for (int index = 0; index < wdViewNum; index++) {
							...
							contentValues.put("entid", qyid);// 污染源(企业)ID
...
//									+ "' and type ='" + task_type_edt_str + "'");
	...
	private String id = "";
	/**当当前记录Guid异常为空时,调用此方法更新到ExelawsTemplet表中*/
...
//		contentValues.put("id", id);
		...
		contentValues.put("billid", guid);
		if (codeStr.equals("0")) {
			contentValues.put("templetname","调查询问笔录"+new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new java.util.Date()));
			contentValues.put("templettype", "XWBL");
		} else if (codeStr.equals("1")) {
			contentValues.put("templetname","现场检查（勘察）笔录"+new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new java.util.Date()));
...
//		contentValues.put("surveypeoplecradcode", task_surveypeoplename_btn_str);
	...
	/** 向数据库Survey_JSTZS(接受通知书)表中字段插入值 */
	public void content_JSTZS_Valput() {
		contentValues = new ContentValues();
		String guid = (String) 通知书编号.getTag();
		...
		contentValues.put("noticeno", 通知书编号.getText().toString());
		contentValues.put("lawtime", currtime);
		contentValues.put("entname", 被查单位.getText().toString());
		contentValues.put("surveytime", 接受调查时间.getTag().toString());
		contentValues.put("breakbehavior", 违法行为.getText().toString());
		contentValues.put("takematerial", 携带资料.getText().toString());
		contentValues.put("contactperson", 现场检查人.getText().toString());
		contentValues.put("address", 现场检查人联系地址.getText().toString());
		contentValues.put("phone", 现场检查人联系电话.getText().toString());
		contentValues.put("updatetime", currtime);
		contentValues.put("signtime", currtime);

		contentValues.put("bjcdz", 被查单位地址.getText().toString());
		contentValues.put("xcjsr", 现场接收人.getText().toString());
		contentValues.put("bjclxdh", 接收人联系电话.getText().toString());
		contentValues.put("fj", 分局.getText().toString());
		contentValues.put("ks", 科室.getText().toString());
		contentValues.put("qtzl", 其它资料.getText().toString());
		contentValues.put("wffl", 违反法律.getText().toString());
	}
...
//		contentValues.put("checkpeople", farendaibiao.getText().toString());
	...
	public void preview_click(View view) {
		...
		if (taskName.contains("勘察") || taskName.contains("询问") || taskName.contains("现场监察记录单") || taskName.equals("环境监察通知书")) {
			preview_save_data();
		} else {
			int result = compare();
			if (result == 1) {
			...
			if (rwbh != null && !rwbh.equals("") && qyid != null && !qyid.equals("") && task_surveypeoplename_btn.getText().toString() != null
				...
				Toast.makeText(TaskSiteEnforcementActivity.this, "执行人名称不能为空", Toast.LENGTH_LONG).show();
			}
		}
	}
	...
	public void selectData(View view) {
		...
		AlertDialog.Builder builder = OtherTools.getAlertDialog(this, "请选择需要携带的资料:", null, new OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				携带资料.setText(buffer.toString().substring(0, buffer.length() - 1));
			}
		}, null, null);
	...
	public void update(final View view) {
		...
		case R.id.start_time_btn:
			DatePickerDialog dpd = new DatePickerDialog(TaskSiteEnforcementActivity.this, dateStart,// 初始化为当前日期
...
//			showDialog(TIME_DIALOG_ID);
		...
		case R.id.one_time_btn:
			DatePickerDialog dpd4 = new DatePickerDialog(TaskSiteEnforcementActivity.this, again_date,// 初始化为当前日期
					dateAndTime.get(Calendar.YEAR), dateAndTime.get(Calendar.MONTH), dateAndTime.get(Calendar.DAY_OF_MONTH));
			isshow = true;
			dpd4.show();
			break;
	...
	public int compare() {
		...
		if (timeStart.compareTo(timeEnd) >= 0) {
			Toast.makeText(TaskSiteEnforcementActivity.this, "请确保结束时间大于开始时间", 1).show();
			return 1;
		} else {
			startTime = timeStart;
			endTime = timeEnd;
...
//				if (bStr.equals("1")) {
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		if (keyCode == KeyEvent.KEYCODE_BACK) {
			if ("现场监察记录单".equals(task_name_tv.getText().toString()) && !"".equals(tv2.getText().toString())) {
				tv2.setText("");
...
//						if (qyzts.equalsIgnoreCase("1")) {
	...
	private void findViewById() {
		...
		/** 设置标题内容 */
		SetBaseStyle(two_list_tool_layout, "现场执法");
		/** 清单检查按钮 */
		...
		task_askedpeopleunit_edt.setText(rwxx.getQYMC_form_GUID(qyid));///----------------被询问人单位
		...
		button_xunwen.setOnClickListener(new Button.OnClickListener() {
			...
			@Override
			public void onClick(View v) {
				new AlertDialog.Builder(TaskSiteEnforcementActivity.this).setTitle("请选择性别").setIcon(
	...
	public void not_edit() {
		task_askedpeoplename_edt.setEnabled(false);//被询问人
		task_askedpeopleidnumber_edt.setEnabled(false);//被询问人身份证号
		task_askedpeopleunit_edt.setEnabled(false);//被询问人工作单位
		//task_askedpeoplesex_edt.setClickable(false);//性别
		
		   for (int i = 0; i < task_askedpeoplesex_edt.getChildCount(); i++) {
			   task_askedpeoplesex_edt.getChildAt(i).setEnabled(false);
	        }
		
		task_askedpeopleage_edt.setEnabled(false);//年龄
		
		
		
		task_askedpeoplepostcode_edt.setEnabled(false);//被询问人邮编
	...
	public void no_edit() {
		接受调查时间.setEnabled(false);
		携带资料.setEnabled(false);
		被查单位地址.setEnabled(false);
		通知书编号.setEnabled(false);
		现场接收人.setEnabled(false);
		接收人联系电话.setEnabled(false);
		违反法律.setEnabled(false);
		违法行为.setEnabled(false);
		分局.setEnabled(false);
		科室.setEnabled(false);
		其它资料.setEnabled(false);
		现场检查人.setEnabled(false);
		现场检查人联系地址.setEnabled(false);
		现场检查人联系电话.setEnabled(false);
...
//		gongzuodanwei.setText("");
	...
	private class QdjcAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			} else if (tname.equals("污染治理设施建设运行情况")) {
	...
	private class ItemAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			} else if (nameString.equals("建设项目执行环保制度情况")) {
			...
			} else if (nameString.equals("排污费缴纳情况")) {
			...
			} else if (nameString.equals("固体废物")) {
