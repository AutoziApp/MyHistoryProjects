...
public class YouYanDetailActivity extends ActivityBase {
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.rightIconB:
                if(value.size()>0&&date.size()>0) {
                    Intent intent = new Intent(YouYanDetailActivity.this, ChartActivity.class);
    ...
    private void readySearch(int btn){
        ...
        switch(btn){
            case FIRSTBTN:
                search( OkHttpUtils.post().url(PathManager.FirDataApp1)
    ...
    private RequestCall intoGetInput(){
        ...
        RequestCall call = OkHttpUtils.post().url(PathManager.DataApp1)
