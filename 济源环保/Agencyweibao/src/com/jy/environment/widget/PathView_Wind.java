...
public class PathView_Wind extends View {
	...
	public PathView_Wind(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		init(context);
	}

	public PathView_Wind(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context);
	}

	public PathView_Wind(Context context) {
		super(context);
		init(context);
	}
	...
	private void drawFrame(Canvas canvas) {
		...
		for (int i = 0; i <= xLineCount; i++) {
			...
			canvas.drawText(Math.round((xMaxValue / xLineCount * (xLineCount - i) - 0.5)) / 10 + "", left,
					startY + bottom / 4, textPaint);
		}
