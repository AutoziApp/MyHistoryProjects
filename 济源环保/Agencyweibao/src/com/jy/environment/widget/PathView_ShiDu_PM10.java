...
public class PathView_ShiDu_PM10 extends View {
	...
	public PathView_ShiDu_PM10(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		init(context);
	}

	public PathView_ShiDu_PM10(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context);
	}

	public PathView_ShiDu_PM10(Context context) {
		super(context);
		init(context);
	}
	...
	private void calculateLeft() {
		...
		case DAY_HOUR:
			...
			// defaultDay =hours;
			for (int i = 0; i < 24; i++) {
				defaultDay[i] = EnvironmentWeatherRankkActivity.shidu[i];
			}
			break;
		case DAY_OLDHOUR:
			yLineCount = 25;
	...
	private void doDraw(Canvas canvas) {
		...
		for (int i = 0; i < data.length; i++) {
			...
			if (i != data.length - 1) {
				...
				int[] colors = { getColorByIntegerValue(dataAqi[i]),
						getColorByIntegerValue(dataAqi[i + 1]) };
			...
			if(i==0){
				int tsize = (int) this.getResources().getDimension(R.dimen.textPaint);
				paintPoint.setTextSize(tsize*2/3);
				canvas.drawText("湿度:"+data[i] + "", x - 10, yPotion - 15, paintPoint);
			}else{
				int tsize = (int) this.getResources().getDimension(R.dimen.textPaint);
				paintPoint.setTextSize(tsize);
		...
		for (int i = 0; i < dataAqiPm10.length; i++) {
			...
			// linkPaint.setStrokeWidth(2);
			if (i != data.length - 1) {
				float nextYPotion = sumHeight2 - dataAqiPm10[i + 1] / tempInterval2
						+ xInterval;
				int[] colors = { getColorByIntegerValuePm10(dataAqiPm10[i]),
						getColorByIntegerValuePm10(dataAqiPm10[i + 1]) };
				LinearGradient linearGradient = new LinearGradient(x + 4,
						yPotion, xPoint.get(i + 1), nextYPotion, colors, null,
						TileMode.REPEAT);
				linkPaint.setShader(linearGradient);
			...
			paintPoint.setAlpha(255);
			// paintPoint.setColor(Color.YELLOW);
			if ((dataAqiPm10[i] > 0) && (dataAqiPm10[i] <= 35)) {
				paintPoint.setColor(Color.parseColor
				("#00FF00"));
			} else if (dataAqiPm10[i] <= 75) {
				paintPoint.setColor(Color.parseColor("#FFFF00"));
			} else if (dataAqiPm10[i] <= 115) {
				paintPoint.setColor(Color.parseColor("#FF7E00"));
			...
			if(i == 0){
				int tsize = (int) this.getResources().getDimension(R.dimen.textPaint);
				paintPoint.setTextSize(tsize*2/3);
				canvas.drawText("PM2.5:"+ dataAqiPm10[i] + "", x - 10, yPotion - 15, paintPoint);
			}else{
				int tsize = (int) this.getResources().getDimension(R.dimen.textPaint);
				paintPoint.setTextSize(tsize);
	...
	private int getColorByIntegerValuePm10(int value) {
		if ((value > 0) && (value < 35)) {
			return (Color.parseColor("#00FF00"));
		} else if (value <= 75) {
			return (Color.parseColor("#FFFF00"));
