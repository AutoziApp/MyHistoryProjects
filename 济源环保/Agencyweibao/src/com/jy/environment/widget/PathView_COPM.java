...
public class PathView_COPM extends View {
	...
	public PathView_COPM(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		init(context);
	}

	public PathView_COPM(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context);
	...
	private void doDraw(Canvas canvas) {
		...
		for (int i = 0; i < data.length; i++) {
			...
			String dataTextString = data[i] + "";
			if (dataTextString.endsWith(".0")) {
				dataTextString = dataTextString.replace(".0", "");
			}
			canvas.drawText(dataTextString, x - 10, yPotion - 15, paintPoint);
		}
	}

	private int getColorByIntegerValue(float value) {
		if ((value > -1) && (value < 51)) {
			return (Color.parseColor("#00FF00"));
		} else if (value < 101) {
			return (Color.parseColor("#FFFF00"));
