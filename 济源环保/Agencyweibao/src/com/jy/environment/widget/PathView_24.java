...
public class PathView_24 extends View {
	...
	public int defaultType = MONITOR_HOUR;
	public String[] defaultDay = new String[24];
	// x,y轴的线条数量
	private int xLineCount = 5;
	private int yLineCount = 25;
	...
	public PathView_24(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		init(context);
	}

	public PathView_24(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context);
	}
	...
	private void calculateLeft() {
		...
		if (defaultType == MONITOR_HOUR) {
			yLineCount = 25;
		}
		MyLog.i(">>>>>>>>>>>>>>xinterv" + xLineCount + ">>>>>>>yinterval"
				+ yLineCount);
		if (xMaxValue == 0)
		...
		// MyLog.i(">>>>>>>>>>>>>>xinterval2"+xInterval+">>>>>>>yinterval"+yInterval);
	}
	...
	private void drawFrame(Canvas canvas) {
		...
		for (int j = 0; j < yLineCount; j++) {
			...
			try {
				canvas.drawText(defaultDay[j - 1], xPoint.get(j-1)-10,
