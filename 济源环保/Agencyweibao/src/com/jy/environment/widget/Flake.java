...
public class Flake {
	 ...
	 * determined.
	 */
	static Flake createFlake(int i, float xRange, Bitmap originalBitmap) {
		Flake flake = new Flake();
		// Size each flake with a width between 5 and 55 and a proportional
		// height
		flake.width = (int) (5 + (float) Math.random() * 15);
...
//		if (i == 0) {
//			flake.y = (flake.height + (float) Math.random() * flake.height) + 10;
//		} else {
			flake.y = -400+(float) Math.random() * 3000;
//		}
		
		// Each flake travels at 50-200 pixels per second
		flake.speed = 50 + (float) Math.random() * 300;
	 ...
	 * determined.
	 */
	static Flake createFlake(int i, float xRange, Bitmap originalBitmap,int snowSize) {
		Flake flake = new Flake();
		// Size each flake with a width between 5 and 55 and a proportional
		// height
		flake.width = (int) (5 + (float) Math.random() * snowSize);
		float hwRatio = originalBitmap.getHeight() / originalBitmap.getWidth();
		flake.height = (int) (flake.width * hwRatio);
...
//		}
		...
		// Each flake travels at 50-200 pixels per second
		flake.speed = 50 + (float) Math.random() * snowSize*10;
