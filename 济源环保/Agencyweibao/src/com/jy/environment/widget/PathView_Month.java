...
public class PathView_Month extends View {
	...
	public PathView_Month(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		init(context);
	}

	public PathView_Month(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context);
	}

	public PathView_Month(Context context) {
		super(context);
		init(context);
	}
	...
	private void init(Context cont) {
		int tsize = (int) this.getResources().getDimension(R.dimen.textPaint)-7;
	...
	private void calculateLeft() {
		...
		case DAY_MONTH:
			...
			for (int i = 0; i < data.length; i++) {
				if (i % 2 == 0) {
					defaultDay[i] = EnvironmentWeatherRankkActivity.months[i];
				} else {
					defaultDay[i] = "";
				}
