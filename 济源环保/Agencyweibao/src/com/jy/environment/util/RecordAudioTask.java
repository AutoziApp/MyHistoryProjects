 ...
 * 
 */
public class RecordAudioTask extends
		AsyncTask<AudioClipListener, Void, Boolean> {
	...
	public RecordAudioTask(Context context, TextView status, TextView log,
			String taskName) {
	...
	protected void onPreExecute() {
		status.setText("״̬" + " for " + getTaskName());
	...
	protected Boolean doInBackground(AudioClipListener... listeners) {
		...
		for (int i = 0; i < 10; i++) {
			try {
				heard = recorder.startRecordingForTime(1000,
	...
	protected void onPostExecute(Boolean result) {
		...
		// redraw perhaps
		if (result) {
			AudioTaskUtil.appendToStartOfLog(log, getTaskName() + " detected "
