...
public class SettingPersonOldActivity extends FragmentActivity implements
		OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_setting_person_old);
	...
	private class GetMonitoringAqiTask extends
		...
		protected void onPreExecute() {
			// TODO Auto-generated method stub
			super.onPreExecute();
			if (!NetUtil.isNetworkConnected(SettingPersonOldActivity.this)) {
				ToastUtil.showShort(SettingPersonOldActivity.this, "无网络");
				try {
					dialog.dismiss();
				} catch (Exception e) {
		...
		protected List<AqiModel> doInBackground(String... params) {
			...
			if (isliebiao == 0) {
				...
				dateEnd = "";
				month = dateMonth;
				url = UrlComponent.getNongDuValue_old;
			} else {
				begin = dateStarr;
				url = UrlComponent.getNongDuValue_old;
			}
	...
	class MyAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (!(city.equals("市平均") || city.equals("县平均"))) {
				convertView = LayoutInflater.from(SettingPersonOldActivity.this)
...
//					holder.item.setBackgroundResource(R.drawable.list_bg_air_avg);
				...
				try {
					if (preferencesUtil.getQuanXian()) {
						...
						holder.aqi.setText(styledText,
								TextView.BufferType.SPANNABLE);
						// 优良天数
						styledText = new SpannableString(
								holder.first_polluction.getText().toString());
						styledText.setSpan(
								new TextAppearanceSpan(
										SettingPersonOldActivity.this,
										R.style.stylebefore),
								cnt.indexOf("/") + 1, cnt.length(),
								Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
						holder.first_polluction.setText(styledText,
								TextView.BufferType.SPANNABLE);
					} else {
						try {
			...
			} else if (isliebiao == 0 || isliebiao == 1) {
				try {
	...
	public void onClick(View v) {
		...
		case R.id.liebiao:
			first_polluction.setVisibility(View.VISIBLE);
			MobclickAgent.onEvent(SettingPersonOldActivity.this, "WeatherShishi");
			isliebiao = 0;
			yubao_head.setVisibility(View.GONE);
			yuce_lin.setVisibility(View.GONE);
			air_head.setVisibility(View.VISIBLE);
		...
		case R.id.center:
			first_polluction.setVisibility(View.VISIBLE);
			MobclickAgent.onEvent(SettingPersonOldActivity.this, "WeatherRibao");
			yubao_head.setVisibility(View.GONE);
			yuce_lin.setVisibility(View.GONE);
			air_head.setVisibility(View.VISIBLE);
			air_headtv1.setText("PM10");
			...
			break;
		case R.id.yubao: {
			MobclickAgent.onEvent(SettingPersonOldActivity.this, "WeatherYubao");
		...
		case R.id.map:
			first_polluction.setVisibility(View.VISIBLE);
			MobclickAgent.onEvent(SettingPersonOldActivity.this, "WeatherYear");
			month_year = 365;
			yubao_head.setVisibility(View.GONE);
			yuce_lin.setVisibility(View.GONE);
			air_head.setVisibility(View.VISIBLE);
...
//				transText(first_polluction, cnt);
		...
		case R.id.tv_month:
			first_polluction.setVisibility(View.VISIBLE);
			MobclickAgent.onEvent(SettingPersonOldActivity.this, "WeatherMonth");
			yubao_head.setVisibility(View.GONE);
			yuce_lin.setVisibility(View.GONE);
			air_head.setVisibility(View.VISIBLE);
			month_year = 30;
		...
		case R.id.statistics_search:
			MobclickAgent.onEvent(SettingPersonOldActivity.this,
					"WeatherSelectTime");
			if (month_year == 365) {
				if (dateStar.equals("") || dateEnd.equals("")) {
					ToastUtil.showShort(SettingPersonOldActivity.this,
							"开始时间或结束时间没有选择");
					return;
				}
	...
	private View getDataPick(final int typeCode) {
		...
		final View view = LayoutInflater.from(SettingPersonOldActivity.this)
...
//		android.os.Process.killProcess(android.os.Process.myPid());
	...
	private void transText(TextView tv, String data) {
		...
		styledText.setSpan(new TextAppearanceSpan(SettingPersonOldActivity.this,
