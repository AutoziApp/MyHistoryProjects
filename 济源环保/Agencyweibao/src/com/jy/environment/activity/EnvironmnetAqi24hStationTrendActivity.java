...
public class EnvironmnetAqi24hStationTrendActivity extends Activity implements
	...
	private TextView monitor_aqi, monitor_polluction, environment_city,
			...
			line_title, monitor_title, ling24_title;
	...
	private void initView() {
		...
		ling24_title = (TextView) findViewById(R.id.ling24_title);
		...
		ling24_title.setText(city + "监测站点趋势图");
		lineTitleArrays.add(city + "监测站点近24小时PM2.5浓度趋势图");
		lineTitleArrays.add(city + "监测站点近24小时PM10浓度趋势图");
		lineTitleArrays.add(city + "监测站点近24小时O3浓度趋势图");
		lineTitleArrays.add(city + "监测站点近24小时SO2浓度趋势图");
		lineTitleArrays.add(city + "监测站点近24小时NO2浓度趋势图");
		lineTitleArrays.add(city + "监测站点近24小时CO浓度趋势图");
		lineTitleArrays.add(city + "监测站点近24小时AQI趋势图");
		polluctionAdapter = new ArrayAdapter<String>(
				EnvironmnetAqi24hStationTrendActivity.this,
			...
			public View getDropDownView(int position, View convertView,
				...
				view = LayoutInflater.from(
						EnvironmnetAqi24hStationTrendActivity.this).inflate(
	...
	class GetMonitorActivity extends
		...
		protected EnvironmentAqiModel doInBackground(String... params) {
			// TODO Auto-generated method stub
			String url = UrlComponent.get24hTrendURL;
		...
		for (int i = 0; i < listShowLineNum.size(); ++i) {
			...
			for (int j = 0; j < monitorModels.get(location).getInfoMonths()
				...
				try {
					...
					String alltime = monitorModels.get(location)
							.getInfoMonths().get(j).getUpdate_time();
					time = alltime.substring(alltime.length() - 8,
							alltime.length() - 3);
					if (time.endsWith(":00")) {
...
//		chart.setCurrentViewport(v, false);
	...
	public void onResume() {
		super.onResume();
		MobclickAgent.onPageStart("EnvironmnetAqi24hStationTrendActivity");
		MobclickAgent.onResume(this);
	}

	public void onPause() {
		super.onPause();
		MobclickAgent.onPageEnd("EnvironmnetAqi24hStationTrendActivity");
