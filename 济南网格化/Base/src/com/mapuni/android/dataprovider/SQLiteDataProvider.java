...
public class SQLiteDataProvider {
		...
		if (insertData != null && insertData.size() > 0) {
			try {
				if (updateOrFetchAllData) {
					// 过滤要删除的数据
					ArrayList<HashMap<String, Object>> temp = filterData(data,
							table);
	...
	private static ArrayList<HashMap<String, Object>> filterData(
			ArrayList<HashMap<String, Object>> data, String table) {
		ArrayList<HashMap<String, Object>> temp = new ArrayList<HashMap<String, Object>>();
		HashMap<String, Object> map = null;
		
	    // 现场检查笔录
		if (table.equals("T_ZFWS_KCBL")) {
			data = parseLowerList(data);
			for (int i = 0; i < data.size(); i++) {
				String sql = "select guid from T_ZFWS_KCBL where taskid = '"
						+ data.get(i).get("taskid") + "' and entcode = '"
						+ data.get(i).get("entcode") + "'";
			...
			data = parseLowerList(data);
			for (int i = 0; i < data.size(); i++) {
				String sql = "select guid from T_ZFWS_XWBL where taskid = '"
						+ data.get(i).get("taskid") + "' and entcode = '"
						+ data.get(i).get("entcode") + "'";
				String guid = SqliteUtil.getInstance().getDepidByDepName(sql);
				if (guid != null && !guid.equals("")) {
	...
	public static boolean synchronizeOneTable(// 将ArrayList封装的数据插入到表中
		...
		if (insertData != null && insertData.size() > 0) {
			try {
				...
				if (count > 0) {
					Log.v(TAG, "synchronizeOneTableLastestData " + table
