...
public class Global extends FrontiaApplication {
	...
	public final static String FILE_SAVEPATH = Environment
			...
			+ "/jinan/Portrait/";
	/** 默认的链接服务器的地址 */
	private String url = "http://192.168.15.155:8080";
	...
	/** 设置用户所属单位 */
	private String userUnit = "辽宁省环境监察局";
	private String userUnitforquestion = "辽宁省环境监察局";
	/** 设置用户名 */
	...
	public static Global getGlobalInstance() {
		return GlobalInstance == null ? GlobalInstance = new Global()
				: GlobalInstance;
	}
...
//		SDKInitializer.initialize(getApplicationContext());
	...
	public void initGlobalData(String user, String pass) {
		...
		ArrayList<HashMap<String, Object>> mapList = users_instance.checkUsers(
				user, pass);
		if (mapList != null) {
			if (mapList.size() > 0) {
				setUsername(user);
				...
				// 加载当前用户的权限集
				HashMap<String, Object> authoritylist = users_instance
						.checkAuthor();
				setAuthoritylist(authoritylist);
				HashMap<String, Object> map = null;
				try {
					map = SqliteUtil.getInstance().getDataMapBySqlForDetailed(
							"select a.[POrgTitle],a.[OrgTitle] from Region a where a.[RegionCode] = '"
									+ getAreaCode() + "'");
	...
	public void initGlobalData(ArrayList<HashMap<String, Object>> mapList) {
		...
		if (mapList != null) {
			if (mapList.size() > 0) {
				...
				setUserid(mapList.get(0).get("UserID").toString());
				setRealName(mapList.get(0).get("U_RealName").toString());
				setUserZW(mapList.get(0).get("ZW").toString());
				setDepId(mapList.get(0).get("DepID").toString());
				setAdministrative(mapList.get(0).get("RegionCode").toString());
				setAreaCode(mapList.get(0).get("SyncDataRegionCode").toString());
				setZfzh(mapList.get(0).get("ZFZH").toString());
				// 加载当前用户的权限集
				HashMap<String, Object> authoritylist = users_instance
						.checkAuthor();
				setAuthoritylist(authoritylist);
