...
public class XcrwActivity extends BaseActivity implements OnClickListener {
	...
	RadioButton rb_has_problem, rb_no_problem, rb_self_solve, rb_shangbao,rb_self_save;// 是否有环境问题，处理上报方式选择按钮
	LinearLayout problemLayout;// 问题布局
	LinearLayout shangbaoLayout;// 选择上报部门的布局
	Spinner sp_xuncha_task;// 巡查任务
	...
	private Double latBaidu;// 纬度
	private Double lonBaidu;// 经度
	// 定位相关
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		SetBaseStyle("巡查上报");
		cameras = new Cameras(XcrwActivity.this);
		initView();
	...
	private void initView() {
		...
		// 查询条件
		mainView = inflater.inflate(R.layout.test, null);
		middleLayout.addView(mainView);
		problem_type_layout = (LinearLayout) mainView.findViewById(R.id.problem_type_layout);
		// 以上均为套路代码
		...
		rb_has_problem = (RadioButton) mainView.findViewById(R.id.rb_has_problem);
		rb_no_problem = (RadioButton) mainView.findViewById(R.id.rb_no_problem);

		problemLayout = (LinearLayout) mainView.findViewById(R.id.problem_layout);
		layout_handling_suggestion = (LinearLayout) mainView.findViewById(R.id.layout_handling_suggestion);
		...
		rb_self_save=(RadioButton) mainView.findViewById(R.id.rb_self_save);

		sp_xuncha_task = (Spinner) mainView.findViewById(R.id.sp_xuncha_task);
		bt_save = (Button) mainView.findViewById(R.id.bt_save);
		bt_save.setOnClickListener(this);
		task_type = (TextView) mainView.findViewById(R.id.myspinner_task_type);
		shangbaoLayout = (LinearLayout) mainView.findViewById(R.id.shangbao_layout);
		...
		edt_problem_description = (EditText) mainView.findViewById(R.id.edt_problem_description);
		//
		...
		rg_clyj.setOnCheckedChangeListener(new OnCheckedChangeListener() {
			...
			public void onCheckedChanged(RadioGroup group, int checkedId) {
				// TODO Auto-generated method stub
				String haveFreeRole = DisplayUitl.readPreferences(XcrwActivity.this, LAST_USER_SP_NAME, "haveFreeRole");
				if (checkedId == R.id.rb_shangbao && haveFreeRole != null && haveFreeRole.equals("1")) {
				...
				if(checkedId==R.id.rb_self_save){
					layout_handling_suggestion.setVisibility(View.GONE);
				}
			}
		});
		...
		initPhotoGrideView();
		initPhotoGrideView1();
		LinearLayout objectLayout = (LinearLayout) mainView.findViewById(R.id.object_layout);
		LinearLayout cutLine = (LinearLayout) mainView.findViewById(R.id.cut_line);
	...
	public void selectData(final View view) {
		...
		AlertDialog.Builder builder = OtherTools.getAlertDialog(XcrwActivity.this, "请选择问题类别:", null,
				new android.content.DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						((TextView) view).setText(buffer.toString().substring(0, buffer.length() - 1));
						edt_problem_description.setText(buffer.toString().substring(0, buffer.length() - 1));

						lastTypePositions.addAll(list);
					}
				}, null, null);
		builder.setMultiChoiceItems(temps_problem_types, null, new OnMultiChoiceClickListener() {
	...
	public void selectData2() {
		Builder builder = new Builder(XcrwActivity.this);
		builder.setTitle("请选择问题类别:");
		// builder.setIcon(XcrwActivity.this.getResources().getDrawable(R.drawable.yutu2));
		LayoutInflater inflater = LayoutInflater.from(XcrwActivity.this);
		View v = inflater.inflate(R.layout.set_txjcr, null);
		final EditText edit_txjcr_dialog = (EditText) v.findViewById(R.id.edit_txjcr_dialog);
		TextView text_chaxun_txjcr = (TextView) v.findViewById(R.id.text_chaxun_txjcr);
			...
			if (jsonObject.getString("status").equals("200")) {
				...
				if (jsonArray.length() > 0) {
					...
					for (int i = 0; i < jsonArray.length(); i++) {
						// 保存父节点
						map_parent.put(jsonArray.getJSONObject(i).getString("itemCode"),
								jsonArray.getJSONObject(i).getString("itemName"));
						// 父节点数据，子节点数据
						groupList.add(jsonArray.getJSONObject(i).getString("itemName"));
						JSONArray jsonArray_childs = jsonArray.getJSONObject(i).getJSONArray("childs");
						if (jsonArray_childs != null && jsonArray_childs.length() > 0) {
							ArrayList<HashMap<String, Object>> list = new ArrayList<HashMap<String, Object>>();
							HashMap<String, Object> map;
							for (int j = 0; j < jsonArray_childs.length(); j++) {
								map = new HashMap<String, Object>();
								map.put("userid", jsonArray_childs.getJSONObject(j).getString("itemCode"));
								map.put("u_realname", jsonArray_childs.getJSONObject(j).getString("itemName"));
								map.put("zfzh", jsonArray_childs.getJSONObject(j).getString("paramCode"));
								list.add(map);
							}
					...
					Toast.makeText(XcrwActivity.this, "暂无数据", 400).show();
					return;
				}
			} else {
				Toast.makeText(XcrwActivity.this, "无法获取问题类别", 400).show();
				return;
			}
		} catch (Exception e) {
		...
		SelectAuditorAdapter adapter = new SelectAuditorAdapter(groupList, childMapData, linkedList, linkedName,
				linkedZfzh, XcrwActivity.this);
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				if (linkedZfzh != null && linkedZfzh.size() > 0) {
					...
					for (int j = 1; j < linkedZfzh.size(); j++) {
						string = string + "," + map_parent.get(linkedZfzh.get(j));
					}
	...
	public void selectData3() {
		...
		try {
			...
			if (jsonObject.getString("status").equals("200")) {
				map_parent = new HashMap<String, String>();
				// 保存父节点
				map_parent.put(jsonObject.getJSONObject("content").getString("itemCode"),
						jsonObject.getJSONObject("content").getString("itemName"));
				// 父节点数据，子节点数据
				groupList.add(jsonObject.getJSONObject("content").getString("itemName"));
		...
		myGridView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
				...
				if (BimpHelper.bmp.size() > Cameras.num) {
					Toast.makeText(XcrwActivity.this, "最多选择" + Cameras.num + "张图片", 400).show();
					return;
				}
				flag = 1;
				BimpHelper.currentFlag = 1;
				if (arg2 == BimpHelper.drr_temp1.size()) {
					new PhotoPopupWindows(XcrwActivity.this, myGridView, cameras);
				} else {
					Intent intent = new Intent(XcrwActivity.this, CheckPicActivity.class);
	...
	private void initPhotoGrideView1() {
		...
		// adapter1.update();
		myGridView1 = (PhotoGridView) mainView.findViewById(R.id.noScrollgridview1);
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		case REQUEST_CODE_GETIMAGE_BYCAMERA:
			if (BimpHelper.drr.size() < Cameras.num && resultCode == -1) {
				try {
					//Bitmap bitmap = MediaStore.Images.Media.getBitmap(this.getContentResolver(), cameras.cropUri);
					Bitmap bitmap = ImageUtil.getThumbnail(XcrwActivity.this, cameras.cropUri, 500);
					Bitmap drawTextToCenter = ImageUtil.drawTextToLeftBottom2(XcrwActivity.this, bitmap,
	...
	public void onClick(View view) {
		...
		case R.id.bt_save:
			...
			if ("other".equals(other)) {
				...
				// if (types.size() > 0) {
				// String str = "";
				// for (int i = 0; i < lastTypePositions.size(); i++) {
				// String s = (String) types.get(
				// Integer.parseInt(lastTypePositions.get(i)))
				// .getItemCode();
				// str += s;
				// if (i != lastTypePositions.size() - 1) {
				// str += ",";
				// }
				// }
				// problemCode = str;
			...
			if (TextUtils.isEmpty(edt_problem_description.getText().toString())) {
				Toast.makeText(XcrwActivity.this, "巡查内容不能为空", 0).show();
				return;
			} else {
				problemDesc = edt_problem_description.getText().toString();
			}
			//办结时必须上传图片
			if (rb_self_solve.isChecked() && layout_handling_suggestion.getVisibility() == View.VISIBLE) {
				if(BimpHelper.drr_temp2.size() < 1) {
					Toast.makeText(XcrwActivity.this, "处理结果中图片不能为空", 0).show();
					return;
				}
			}
			String status = "1";
...
//			if (null == point) {
//				Toast.makeText(XcrwActivity.this, "位置信息不能为空", 0).show();
//				return;
//			}
			if (rb_has_problem.isChecked()) {
				if (task_type.getText().toString().trim().isEmpty()) {
					Toast.makeText(XcrwActivity.this, "问题类别不能为空", 0).show();
					return;
				}
			}
			if (!isUping) {
				isUping=true;
				try {
					uploadFile(uphandler, drr1, drr2, point, taskId, patrolObjectId, isHaveProblem, problemDesc, opinion,
				...
				Toast.makeText(XcrwActivity.this, "问题正在上传请耐心等候", 0).show();
			}
	...
	private Handler uphandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			switch (msg.what) {
			case 101:
				yutuLoading = new YutuLoading(XcrwActivity.this);
			...
			case 102:
				...
				isUping=false;
				Toast.makeText(XcrwActivity.this, "执行成功", 1000).show();
				finish();
				break;
			case 103:
				if (null != yutuLoading) {
				...
				isUping=false;
				Toast.makeText(XcrwActivity.this, "执行失败", 1000).show();
				break;
			case 104:
				mbwz_eit.setText(addr);
				break;
			default:
				break;
			}
...
//			this.finish();
	...
	public void uploadFile(final Handler uphandler, List<String> list1, List<String> list2, GeoPoint point,
			String taskId, String patrolObjectId, String isHaveProblem, String problemDesc, String opinion,
			String status, String handlerGrid, String problemCode, String createTime) throws Exception {
		...
		MultipartBody.Builder builder = new MultipartBody.Builder().setType(MultipartBody.FORM)
				...
				.addFormDataPart("taskId", taskId) // ok
				...
				// .addFormDataPart("createId", createId)//ok 不需要
				// .addFormDataPart("patrolRecordId", patrolRecordId) 不需要
				.addFormDataPart("problemCode", problemCode)
				// ok
				.addFormDataPart("sessionId",
						DisplayUitl.readPreferences(XcrwActivity.this, LAST_USER_SP_NAME, "sessionId"))// 必须
		...
		for (String path : list2) {
			File file = new File(path);
			builder.addFormDataPart("file1", file.getName(), RequestBody.create(MEDIA_TYPE_MARKDOWN, file));
		}
	...
	private void getObjectData() {
		final YutuLoading yutuLoading = new YutuLoading(XcrwActivity.this);
		yutuLoading.setCancelable(false);
		...
		String sessionId = DisplayUitl.readPreferences(XcrwActivity.this, LAST_USER_SP_NAME, "sessionId");
		...
		utils.send(HttpMethod.GET, url/* , params */, new RequestCallBack<String>() {
			...
			public void onFailure(HttpException arg0, String arg1) {
				...
				Toast.makeText(XcrwActivity.this, "数据请求失败", 200).show();
			}
	...
	private void getProblemType() {
		...
		String url = "";
		if ("other".equals(other)) {
			url = PathManager.BASE_URL_JINNAN + "/JiNanhuanbaoms/param/supervise_type/all.do?sessionId=" + sessionId;
		} else {
			url = PathManager.BASE_URL_JINNAN + "/JiNanhuanbaoms/param/" + other + "/items.do?sessionId=" + sessionId;
		}
						...
						if (isFirst) {
							...
							if (bundle != null) {
								...
								for (int i = 0; i < Objects.size(); i++) {
									if (code.equals(Objects.get(i).getId())) {
										sp_jianguan_object.setSelection(i);
									}
								}
							}
						}
	...
	public void selectShangbaoBuMenData() {
		...
		final ExpandableListView listView = (ExpandableListView) v.findViewById(R.id.expList_txjcr);
		builder.setView(v);
		myAdapter = new MyExpandableListViewAdapter(childrenListMap, parentList, XcrwActivity.this);
		...
		listView.setOnChildClickListener(new OnChildClickListener() {
			...
			public boolean onChildClick(ExpandableListView parent, View v, int groupPosition, int childPosition,
					long id) {
				// TODO Auto-generated method stub
				handlerGrid = shangBaoBuMen.getRows().get(groupPosition).getChilds().get(childPosition).getCode();
	...
	private void getShangBaoBuMen() {
		...
		utils.send(HttpMethod.GET, url, new RequestCallBack<String>() {
			...
			public void onSuccess(ResponseInfo<String> arg0) {
				...
				if ("200".equals(shangBaoBuMen.getStatus())) {//
					...
					Log.v("lfwang", "获取上报部门失败" + shangBaoBuMen.getMessage());
				}
			}
		});
	}
	...
	public class SelectAuditorAdapter extends BaseExpandableListAdapter {
		...
		public SelectAuditorAdapter(List<String> groupList, ArrayList<ArrayList<HashMap<String, Object>>> childMapData,
				LinkedList<String> usersb, LinkedList<String> linkedName, LinkedList<String> linkedZfzh,
				Context context) {
		...
		public View getChildView(int groupPosition, int childPosition, boolean isLastChild, View convertView,
			...
			final String zfzh = childMapData.get(groupPosition).get(childPosition).get("zfzh").toString();
	...
	public class MyLocationListenner implements BDLocationListener {
		...
		public void onReceiveLocation(BDLocation location) {
			...
			if (result == 61 || result == 65 || result == 66 || result == 67
					|| result == 68 || result == 161) {
				if(location != null && location.getLongitude() != 1 && location.getLatitude() != 1) {
					lonBaidu = location.getLongitude();
					latBaidu = location.getLatitude();
					String locationDescribe = location.getLocationDescribe();
					if (locationDescribe != null && locationDescribe.length() > 0 && locationDescribe.substring(0, 1).equals("在")) {
